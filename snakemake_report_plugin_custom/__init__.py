...
class Reporter(ReporterBase):
    def __post_init__(self):
        ...
        self.results_template_files = {
            what: f"results_{what}.md" for what in ("main", "section", "subsection")
        ...
        if self.settings.config is not None:
            ...
            if "results" in self.settings.config and self.settings.results is None:
                self.settings.results = Path(
                    self.settings.config["results"].get("path")
                )
                if self.settings.results is None:
                    logger.warning(
                        "reporting config section 'results' found, but no path "
                        "specified - no results report is generated"
                    )
            if "resources" in self.settings.config and self.settings.resources is None:
                self.settings.resources = Path(
                    self.settings.config["resources"].get("path")
                )
                if self.settings.resources is None:
                    logger.warning(
                        "reporting config section 'resources' found, but no path "
                        "specified - no resources report is generated"
                    )

        if self.settings.results is None and self.settings.resources is None:
            raise WorkflowError(
                "either specify results or resources report paths in config or with "
                "--report-custom-results or --report-custom-resources parameters"
            )
    ...
    def render_resources_report(self):
        ...
        if suffix == "":
            ...
            raise NotImplementedError(
                f"reports of type {suffix} are not supported")
    ...
    def render_results_report(self):
        ...
        render_results_html(
            ...
            config_dict,
            self.settings.config.get("results", {}),
            output_dir,
            self.template_dir,
            self.results_template_files,
            embedded=False,
