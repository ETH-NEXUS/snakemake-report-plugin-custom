...
class Reporter(ReporterBase):
    def __post_init__(self):
        ...
        if self.settings.results is None and self.settings.resources is None:
            raise WorkflowError("either specify --report-custom-results or --report-custom-resources")
    ...
    def explore_content(self):       
        ...
        for attr in dir(self):
            ...
            if isinstance(value, list):
                for item in value:
                    logger.info(item if not hasattr(item, '__dict__') else vars(item))
            elif isinstance(value, dict):
                for key, item in value.items():
                    logger.info(f"{key}: {item if not hasattr(item, '__dict__') else vars(item)}")
            else:
                    logger.info(value if not hasattr(value, '__dict__') else vars(value))
    ...
    def print_content(self):
        ...
        for job in self.jobs:
            logger.info((job))  # Convert job object to a dictionary if possible
        ...
        for rule_name, rule_record in self.rules.items():
            ...
            logger.info(rule_record)  # Convert rule record to dictionary if possible
        ...
        logger.info("\n--- SETTINGS ---")
        logger.info(vars(self.settings))  # Convert settings object to a dictionary if possible
    ...
    def render_resources_report(self):
        ...
        for rule in workflow.rules:
            ...
            if rule.benchmark is not None:
                ...
                file, infos= create_benchmark_plot(rule.name, rule.benchmark, 
                                                   rule.input, output_dir)
