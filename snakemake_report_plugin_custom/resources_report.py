...
def create_benchmark_plot(rule_name, benchmark_file, input_names, out_path):
    ...
    for bm_file in glob(benchmark_glob):
        ...
        input_files = [substitute_wildcards(in_file, wildcards) for in_file in input_names]
        try:
            input_size = sum(path.getsize(in_file) for in_file in input_files)
        except FileExistsError as e:
            logger.error("Cannot find input filed for wildcard "
                         f"{wildcard_string}\n{e.message}")
            input_size=0
        # read snakemake benchmark file
        stats = pd.read_csv(bm_file, sep="\t")
        resources["_".join(wildcards.values())] = {"mem": stats['max_rss'][0], "time": stats['s'][0], "input_size": input_size}
    if not resources:
    ...
    ax1.scatter(df['input_size']/2**30, df['mem']/2**10, color='blue', label='Max RSS', marker='o')
    ax1.set_xlabel('Input Size (GB)')
    ax1.set_ylabel('used memory (GB)', color='blue')
    ax1.tick_params(axis='y', labelcolor='blue')
    # Create a second y-axis for time
    ax2 = ax1.twinx()
    ax2.scatter(df['input_size']/2**30, df['time']/60, color='red', label='Time (min)', marker='x')
