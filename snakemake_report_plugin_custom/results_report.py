...
# monkey patching of the snakemake FileRecord class
# which gets passed to respective templates as variables
...
MD_TABLE_SETTINGS = [
    ...
    # Alignment for numeric columns ("left", "center", "right")
    "numalign",
...
def _render_file_record_default(self):
    """
    Generate a string representation of the FileRecord instance.

    This function is used for the __str__() method of the FileRecord class.
    It creates a short string representation of the FileRecord instance by
    listing its attributes and their values. If an attribute value is longer
    than 12 characters, it is truncated.

    Returns:
        str: A string representation of the FileRecord instance.
    """
    if not hasattr(self, "render_settings"):
        ...
        attribute_list = [
            f"{attr}={short(str(getattr(self, attr)))}"
...
def _render_file_record(self, **params):
    """
    Render the FileRecord instance as markdown.

    This function renders the FileRecord instance based on the settings
    specified in the render_settings attribute. It supports rendering
    as an image with specific HTML attributes.

    Args:
        **params: Additional parameters to override the default render settings.

    Returns:
        list: A list of rendered markdown strings.
    """
    rendered = []
    ...
    if not self.render_settings.get("hide", False):
        if self.render_settings.get("type") == "image":
            ...
            if any(p in params for p in HTTP_IMAGE_SETTINGS):
                ...
                rendered.append(
                    f'<img src="{self.path}"{"".join(settings_list)}>')
            else:
            ...
            if "col_select" in params:
                try:
                    ...
                    logger.error(
                        f'ignoring col_select="{params["col_select"]}" '
                        f'for {self.path.name}: {e}'
                    )
            rendered.append(
                df.to_markdown(
                    **{k: v for k, v in params.items() if k in MD_TABLE_SETTINGS}
                )
            )  # or to_html()
...
def set_render_settings(self, params, presets):
    ...
    if not params:
        params = {
            **presets.get(render_type, {}).get("default", {}),
        ...
        try:
            ...
            logger.error(
                f'no preset "{params}" for files of type "{render_type}"')
            params = {}
...
FileRecord.default_render_settings = {
    "table": {"sep": {"cnv": ",", "tsv": "\t"}, "show_link": True},
    "image": {"show_link": False},
    "other": {"show_link": True},
}
# overwrite general settings with specific settings
...
def remove_category(results, cat_name, subcat_name=None):
    "Remove a category or subcategory from the results."
    subcat = None
...
def render_results_html(
    ...
    """Renders the results report using the provided template and config files.
    Args:
        results (dict): A dictionary containing the results of the workflow.
        workflow_description (str): A description of the workflow, generated
        from rst provided in report directive.
        snakemake_config (dict): The Snakemake config dict.
        report_config (dict): The report config dict.
        output_dir (str): The output directory for the results report.
        template_dir (str): The directory containing the template files.
        template_file (dict): A dictionary containing the names of the template files.
        embedded (bool): A flag indicating whether the results report should be
        embedded in the main report.
    """
    ...
    workflow_description = load_workflow_description(
        env, report_config, workflow_description, Snakemake, results
    )
    toc = report_config.get("result_sections", {})
    # define section order and settings from report config and remove skipped sections
    process_toc(toc, results)
    ...
    # load render presets from report config
    default_settings_config = report_config.get("render_options")
    render_presets, default_suffix_params = load_render_presets(
        default_settings_config)

    # prepare and render each category and subcategory
    for category, subcat_dict in results.items():
        process_category(
            ...
            category_order,
            default_suffix_params,
            render_presets,
            Snakemake,
        )

    # copy result files to output directory
    _ = copy_result_files(results, output_dir)

    # render the report
    report_content = template.render(
        title=report_config.get("title", "Result Report"),
        results=dict(
            sorted(results.items(), key=lambda x: (x[0].sort_key, x[0].name))),
        now=now,
    ...
    write_report(output_dir, report_content)


def load_workflow_description(
    env, report_config, workflow_description, Snakemake, results
):
    description_file = report_config.get("description")
    if description_file:
        logger.debug(f"Custom workflow description from {description_file}")
        # render the template consistent with snakemake workflow description rendering
        workflow_description = render_template(
            env, description_file, snakemake=Snakemake, categories=results, files={}
        )
    return workflow_description


def process_toc(toc, results):
    if not toc:
        return
...
def load_render_presets(default_settings_config):
    render_presets = {}
    default_suffix_params = {
        render_type: params.get("suffix", [])
        for render_type, params in default_settings_config.items()
    }
    for render_type in default_settings_config:
        ...
        for ps_name, ps_vals in presets.items():
            render_presets[render_type][ps_name] = {
                **default_settings_config[render_type],
                **ps_vals,
            }
    return render_presets, default_suffix_params
...
def process_category(
    ...
    category.sort_key = category_order.get(
        category.name, len(category_order) + 1)
    cat_settings = {
        k: v for k, v in toc.get(category.name, {}).items() if k != "subsections"
    }
    cat_suffix_params = {
        render_type: params.get("suffix", [])
        for render_type, params in cat_settings.items()
        if isinstance(params, dict)
    }
    subcategory_order = {
        cat: i + 1 for i, cat in enumerate(cat_settings.get("subsections", {}))
    }
    subcategory_order["Other"] = 0
    for subcat, result_list in subcat_dict.items():
        subcat_settings = (
            toc.get(category.name, {}).get(
                "subsections", {}).get(subcat.name, {})
        )
        process_subcategory(
            subcat,
            result_list,
    ...
    category.rendered = section_template.render(
        category=category,
        subcategories=dict(
            sorted(subcat_dict.items(), key=lambda x: (
                x[0].sort_key, x[0].name))
        ),
        has_other=any(subcat.is_other for subcat in subcat_dict.keys()),
        snakemake=Snakemake,
    )
...
def process_subcategory(
    ...
    subcat.sort_key = subcategory_order.get(
        subcat.name, len(subcategory_order) + 1)
    subcat_suffix_params = {
        render_type: params.get("suffix", [])
        for render_type, params in subcat_settings.items()
        if isinstance(params, dict)
    }
    suffix_params = {
        **default_suffix_params,
    ...
    logger.debug(
        f"preset settings for {category.name} {subcat.name}: {suffix_params}")

    for file in result_list:
        file.set_default_render_settings(suffix_params)
        file.set_render_settings(cat_settings, render_presets)
        file.set_render_settings(subcat_settings, render_presets)
    ...
    subcat.rendered = subsection_template.render(
        subcategory=subcat, files=result_list, snakemake=Snakemake
    )


def write_report(output_dir, report_content):
...
def load_template(env, template_path):
    try:
        ...
        logger.error(
            f"Template at path {template_path} not found. Use Default template."
        )
        raise
